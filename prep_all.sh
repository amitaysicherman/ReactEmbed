...
for m_task in "${mol_task[@]}"; do
  for m_model in "${molecule_models[@]}"; do
    python eval_tasks/prep_tasks.py --task "$m_task" --m_model "$m_model"
  done
done
...
for p_task in "${prot_task[@]}"; do
  for p_model in "${protein_models[@]}"; do
    python eval_tasks/prep_tasks.py --task "$p_task" --p_model "$p_model"
  done
done
...
for l_task in "${lig_task[@]}"; do
  for p_model in "${protein_models[@]}"; do
    for m_model in "${molecule_models[@]}"; do
      python eval_tasks/prep_tasks.py --task "$l_task" --p_model "$p_model" --m_model "$m_model"
    done
  done
done